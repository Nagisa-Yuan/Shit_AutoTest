...
from PyQt5.QtWidgets import QApplication, QWidget, QVBoxLayout, QHBoxLayout, QPushButton, QComboBox, QTableWidget, \
    QTableWidgetItem, QStackedWidget, QLabel
from PyQt5.QtDesigner import QPyDesignerCustomWidgetPlugin
from PyQt5 import QtCore, QtWidgets


####################################################### 电压源模式绘图组件 #####################################################
class voltagePlotWidget2430(QWidget):
    ...
    def save_to_excel(self):
        ...
        try:
            ...
            # 更新目标工作表数据
            existing_sheets['Measurements2430'] = df_to_save.reindex(
...
######################################################################电流源模式绘图###############################################################
################################################################################################################################################
class currentPlotWidget2430(QWidget):
    def __init__(self):  # 类初始化，继承 QWidget 创建自定义 Qt 控件，用于嵌入 Matplotlib 图形
        super().__init__()
        self.initUI()
        self.currentPlotType = "line"  # 默认显示折线图
...
class PulsePreviewWidget2430(QWidget):
    def __init__(self, parent=None):
        ...
        self.min_pulse_width = 80e-6  # 图示最小脉宽开销80μs
        # self.min_off_time = 2.9e-3  # 图示最小输出关闭时间2.9ms
        self.min_off_time = 0  # 图示最小输出关闭时间0ms
        self.meas_sig_time = 60e-6  # 图示信号测量时间60μs

        # 初始化仪器参数
        self.pulse_params = {
            'high_level': 1.0,          # 高电平 (V/A)
            'pulse_width': 1e-3,        # 脉宽 (需≥80μs)
            'pulse_delay': 1e-3,        # 脉冲间隔
            'arm_count': 1,             # 臂触发次数
            'trigger_count': 5,          # 触发次数（总脉冲数=arm*trigger）
            'source_mode': 'voltage',   # 源模式
            'meas_ref_zero': False       # 是否包含参考/零测量（影响Off-Time）
        }
...
#######################################基础脉冲模式######################################
    def update_preview(self, params):
        ...
        try:
            # 参数验证与更新
            self._validate_and_update_params(params)
            # 生成时序数据
            time_axis, waveform = self._generate_timing_sequence()
            # 动态绘制波形
            self._render_waveform(time_axis, waveform)
        except ValueError as ve:
            print(f"参数错误: {str(ve)}")
            self._clear_display()
        except Exception as e:
            print(f"波形生成异常: {str(e)}")
            self._clear_display()



    def _validate_and_update_params(self, params):
        """带约束条件的参数验证"""
        # 必填参数检查
        required_keys = ['high_level', 'pulse_width', 'pulse_delay',
                        'arm_count', 'trigger_count']
        if not all(k in params for k in required_keys):
            raise ValueError("缺少必要脉冲参数")

        # 确保所有参数都是数值类型
        try:
            params['pulse_width'] = float(params['pulse_width'])
            params['pulse_delay'] = float(params['pulse_delay'])
            params['arm_count'] = int(params['arm_count'])
            params['trigger_count'] = int(params['trigger_count'])
        except (TypeError, ValueError) as e:
            raise ValueError(f"参数类型错误: {str(e)}")

        # 参数范围校验（基于图5-2）
        if params['pulse_width'] < self.min_pulse_width:
            raise ValueError(f"脉宽不得小于{self.min_pulse_width*1e6:.0f}μs")
        # if params['pulse_delay'] < self.min_off_time:
        #     raise ValueError(f"脉冲间隔不得小于{self.min_off_time*1e3:.1f}ms")
        if params['arm_count'] < 1 or params['trigger_count'] < 1:
            raise ValueError("触发次数必须≥1")

        # 更新参数容器
        self.pulse_params.update(params)

    def _generate_timing_sequence(self):
        """生成符合仪器时序的波形数据"""
        # 计算单周期时间（图示时序结构）
        total_off_time = self.pulse_params['pulse_delay']
        total_off_time += 3.3e-3  # 2 NPLC/frequency
        if self.pulse_params['meas_ref_zero']:
            total_off_time += 2.9e-3    # Minimum output off time overhead
        else:
            total_off_time += 1.4e-3    # 禁用自动归零减小延时


        period = self.pulse_params['pulse_width'] + total_off_time + 80e-6 + 3.36e-3
        total_pulses = self.pulse_params['arm_count'] * self.pulse_params['trigger_count']

        # 生成时间轴（动态分辨率）
        t = np.linspace(0, period * total_pulses, 2000)
        waveform = np.zeros_like(t)

        # 构建脉冲序列（含测量阶段）
        for n in range(total_pulses):
            # 脉冲开启阶段
            pulse_start = n * period
            pulse_end = pulse_start + self.pulse_params['pulse_width']
            mask = (t >= pulse_start) & (t < pulse_end)
            waveform[mask] = self.pulse_params['high_level']

            # 测量阶段可视化（图示Meas Sig区域）
            if self.pulse_params['meas_ref_zero']:
                meas_start = pulse_end + 60e-6  # 图示60μs延迟
                meas_end = meas_start + self.meas_sig_time
                mask = (t >= meas_start) & (t < meas_end)
                waveform[mask] = 0.8 * self.pulse_params['high_level']  # 测量阶段半高显示

        return t, waveform

    def _render_waveform(self, t, waveform):
        """动态渲染波形图"""
        self.ax.clear()

        # 主波形绘制
        self.ax.plot(t * 1e3, waveform, 'b-', lw=1.5, label='Output')

        # 测量阶段标记（图示Meas Sig）
        if self.pulse_params['meas_ref_zero']:
            self.ax.fill_between(t * 1e3, 0, waveform.max(),
                                 where=(waveform == 0.8 * self.pulse_params['high_level']),
                                 color='lightgreen', alpha=0.3, label='Measurement')

        # 标注关键时间参数
        text_str = f"PW: {self.pulse_params['pulse_width'] * 1e3:.1f}ms\n" \
                   f"Delay: {self.pulse_params['pulse_delay'] * 1e3:.1f}ms\n" \
                   f"Total Pulses: {self.pulse_params['arm_count'] * self.pulse_params['trigger_count']}"
        self.ax.text(0.95, 0.95, text_str, transform=self.ax.transAxes,
                     ha='right', va='top', bbox=dict(facecolor='white', alpha=0.8))

        # 坐标轴设置
        self.ax.set_xlabel('Time (ms)')
        ylabel = 'Voltage (V)' if self.pulse_params['source_mode'] == 'voltage' else 'Current (A)'
...
####################################### 脉冲扫描模式 ########################################
    ...
    def update_sweep_preview(self, params):
        """更新脉冲扫描波形预览"""
        try:
            # 参数验证与更新
            self._validate_and_update_sweep_params(params)
            # 生成时序数据
            time_axis, waveform = self._generate_sweep_timing_sequence()
            # 动态绘制波形
            self._render_sweep_waveform(time_axis, waveform)
        except ValueError as ve:
            print(f"参数错误: {str(ve)}")
            self._clear_display()
        except Exception as e:
    ...
    def _validate_and_update_sweep_params(self, params):
        """带约束条件的参数验证（扫描模式专用）"""
        # 必填参数检查
        required_keys = [
            'PulseWidth', 'Delay', 'SourceMode',
            'StartLevel', 'StopLevel', 'StepLevel'
        ]
        if not all(k in params for k in required_keys):
            raise ValueError("缺少必要扫描参数")

        # 基础参数类型转换
        try:
            self.pulse_params['pulse_width'] = float(params['PulseWidth'])
            self.pulse_params['pulse_delay'] = float(params['Delay'])
            self.pulse_params['source_mode'] = params['SourceMode'].lower()
            start = float(params['StartLevel'])
            stop = float(params['StopLevel'])
            step = float(params['StepLevel'])
        except (TypeError, ValueError) as e:
            raise ValueError(f"参数类型错误: {str(e)}")

        # 步进有效性检查
        if step == 0:
            raise ValueError("步进值不能为零")
        if (stop - start) * step <= 0:
            raise ValueError("无效的起始/终止值组合")

        # 计算触发次数
        self.pulse_params['trigger_count'] = int(abs(stop - start) / abs(step)) + 1
        self.pulse_params['high_level'] = start
        self.pulse_params['sweep_step'] = step

    def _generate_sweep_timing_sequence(self):
        """生成扫描模式时序数据"""
        # 基础时间参数
        pulse_width = self.pulse_params['pulse_width']
        pulse_delay = self.pulse_params['pulse_delay']
        step = self.pulse_params['sweep_step']
        total_pulses = self.pulse_params['trigger_count']

        # 计算单周期时间
        period = pulse_width + pulse_delay + self.min_off_time
        ...
        for n in range(total_pulses):
            # 当前电平值
            current_level = self.pulse_params['high_level'] + n * step

            # 脉冲开启阶段
            pulse_start = n * period
            pulse_end = pulse_start + pulse_width
            mask = (t >= pulse_start) & (t < pulse_end)
            waveform[mask] = current_level

            # 测量阶段标记（与基本模式一致）
            if self.pulse_params['meas_ref_zero']:
                meas_start = pulse_end + 60e-6
                meas_end = meas_start + self.meas_sig_time
                mask = (t >= meas_start) & (t < meas_end)
                waveform[mask] = 0.8 * current_level  # 半高显示测量阶段

        return t, waveform

    def _render_sweep_waveform(self, t, waveform):
        """渲染扫描模式波形图"""
        self.ax.clear()

        # 主波形绘制（阶梯状）
        self.ax.step(t * 1e3, waveform, 'b-', where='post', lw=1.5, label='Output')

        # 标注扫描参数
        text_str = (
            f"Start: {self.pulse_params['high_level']:.2f}\n"
            f"Stop: {self.pulse_params['high_level'] + (self.pulse_params['trigger_count'] - 1) * self.pulse_params['sweep_step']:.2f}\n"
            f"Step: {self.pulse_params['sweep_step']:.2f}\n"
            f"Width: {self.pulse_params['pulse_width'] * 1e3:.1f}ms\n"
            f"Delay: {self.pulse_params['pulse_delay'] * 1e3:.1f}ms"
        )
        self.ax.text(0.95, 0.15, text_str, transform=self.ax.transAxes,
...
#################################################################### 扫描模式表格 ###############################################################
################################################################################################################################################
class PulseResultWidget2430(QWidget):
    def __init__(self):
        super().__init__()
        self.initUI()
        self.loadData()
    ...
    def initUI(self):
        ...
        self.mode_combo.addItems(["V_Pulse", "I_Pulse"])
    ...
    def get_current_sheet(self):
        """获取当前选择的sheet名称"""
        # print("2430Voltage" if self.mode_combo.currentText() == "电压扫描" else "2430Current")
        return "VPulse" if self.mode_combo.currentText() == "V_Pulse" else "IPulse"
...
class SweepWidget2430(QWidget):
    ...
    def get_current_sheet(self):
        """获取当前选择的sheet名称"""
        print("2430Voltage" if self.mode_combo.currentText() == "电压扫描" else "2430Current")
        return "2430Voltage" if self.mode_combo.currentText() == "电压扫描" else "2430Current"
